...
_CHEAP_TOOLS = frozenset(
    {"read_logs", "query_flake_history", "recent_commits", "check_owner", "inspect_test_code", "cluster_metrics"}
)
_EXPENSIVE_TOOLS = frozenset({"rerun_test", "run_diagnostic", "file_bug", "ping_owner", "quarantine_test"})


class InvestigationReward(RewardComponent):
    """Shaping reward for investigation quality: coverage, ordering, and redundancy.

    coverage  = fraction of scenario's informative_tools that were called (weight 0.6).
    ordering  = 1.0 minus penalty for cheap-after-expensive violations (weight 0.2).
    redundancy = -0.1 per duplicate (tool_name, args) call.
    Score range: [-1.0, 1.0].
    """
    ...
    def score(self, trace: EpisodeTrace, scenario: Scenario) -> ComponentScore:
        called_tools = [
            rec.action.tool_name for rec in trace.episode.history if isinstance(rec.action, ToolCall)
        ]
